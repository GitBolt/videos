...
def RentIntro(self: Scene):
    ...
    source = Text(
        "Source: Validators.app", color=WHITE, font_size=30, weight=BOLD)
    source.move_to(UP * 1)
    ...
    rent_value.add_updater(lambda z: z.become(Text("Rent: " + str(round(x.get_value() * 0.00003784, 5)) + " SOL", color=PINK, font_size=32, weight=BOLD))
                           .next_to(rent, RIGHT * 2))

    data.add_updater(lambda z: z.become(
        Text(str(round(x.get_value())) + " bytes", font_size=23, color=BLUE).move_to(data.get_center())))

    self.add(disclaimer)
    self.play(x.animate.set_value(64), run_time=5, run_func=slow_into)
    self.wait()
    self.camera.frame.save_state()
    self.play(self.camera.frame.animate.move_to(RIGHT * 5).scale(1.3),
              Uncreate(disclaimer), Uncreate(arrow), Uncreate(rent_value))
    ...
    lamports.add_updater(lambda z: z.become(Text(str(
        round(x.get_value() * 10) + 19), font_size=23).move_to(lamports.get_center())))

    progress_bar.add_updater(lambda z: z.become(
        Line(color=PINK, start=initial_start, end=dot.get_center())))

    self.play(x.animate.set_value(rent2[1].get_left()[
        0] + 0.25), run_time=3, rate_func=linear)
