...
def TokenAccounts(self: Scene):
    ...
    owner.become(
        Text("Token...Q5DA", font_size=20).move_to(owner.get_center()))

    token_account = system_account.copy()
    token_account_t = system_account_t.copy()

    newusdc = SVGMobject("assets/usdc.svg").scale(0.2)
    newusdc.next_to(token_account[2], LEFT)

    self.play(FadeOut(usdt), Transform(
        usdc, token_account))
    ...
    arrow = Arrow([-1.1, 1.2, 0], metadata.get_left(),
                  path_arc=50 * DEGREES)
    ...
    self.play(account1.animate.move_to(LEFT * 4.5 + DOWN * 2).scale(0.6),
    ...
    metadata_dot = Dot()
    metadata_arrow = Arrow(tokenAccount.get_right(),
                           metadata,
                           path_arc=20*DEGREES,
                           stroke_width=3,
    ...
    owner_dot = Dot()
    owner_arrow = Arrow([3.8, 2.6, 0], account2.get_top(), path_arc=-20 *
                        DEGREES, stroke_width=3, max_tip_length_to_length_ratio=0.1)
    ...
    display_arrow = Arrow(metadata, screen_amount.get_top(),
    ...
    self.play(usdc_value.animate.move_to(
        tokenAccount.get_left()).scale(0.7))
    self.play(FadeOut(usdc_value), run_time=0.8)

    self.play(metadata.animate.become(
        make_code("""
            "isNative":false,
            "mint":"Es9v...nwNYB",
            "owner":"B3Bh...44ai",
        ...
        """, "json").move_to(metadata.get_center())
