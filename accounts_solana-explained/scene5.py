...
def RunSOLAccountsToEth(self: MovingCamera):
    ...
    move_along_animation = MoveAlongPath(
        dot, rect_path, run_time=10, rate_func=linear)
    ...
    def update_text(obj, dt):
        ...
        if elapsed_time >= delay:
            obj.become(Text(random.choice(random_data),
                            font_size=20).move_to(data_field2.get_center()))
            elapsed_time = 0
    ...
    self.wait(0.1)
    self.play(Transform(code, data_field),
              move_along_animation,
              )

    self.camera.frame.save_state()

    self.play(self.camera.frame.animate.scale(2).shift(
        RIGHT * 20), move_along_animation)

    data_field2.clear_updaters()

    self.remove(*[obj for obj in self.mobjects])

    eth_acc, eth_acc_t = create_table([["Field", "Data"],
                                       ["nonce", "12"],
                                       ["balance", "10"],
                                       ["storage", "userid: 1"],
                                       ["code", "evmcode.bin"],
